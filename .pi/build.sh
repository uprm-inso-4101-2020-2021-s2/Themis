...
# docker buildx build --tag Themis --file .pi/Dockerfile --platform linux/arm64 --cache-from Themis:cache --cache-to Themis:cache --load --progress tty .

# save the image to result-rootfs.tar
docker save --output ./.tmp/result-rootfs.tar Themis
...
sudo bash -c "echo Themis > ./.tmp/result-rootfs/etc/hostname"
...
# Make the image file
fallocate -l 4G "images/Themis.img"

# loop-mount the image file so it becomes a disk
sudo losetup --find --show images/Themis.img
