...
func (k Keeper) GroupAddress(c context.Context, req *types.QueryGetGroupAddressRequest) (*types.QueryAllGroupResponse, error) {
	if req == nil {
		return nil, status.Error(codes.InvalidArgument, "invalid request")
	}
	...
	store := ctx.KVStore(k.storeKey)
	groupStore := prefix.NewStore(store, types.KeyPrefix(types.GroupAddrKey))

	pageRes, err := types.PrefixPaginate(groupStore, types.KeyPrefix(req.Addr), req.Pagination, func(key []byte, value []byte) error {
		var group types.Group
		if err := k.cdc.UnmarshalBinaryBare(value, &group); err != nil {
			return err
		}
